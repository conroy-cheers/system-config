    ...
    let
      ...
      rageImportEncryptedRaw =
        ...
        importOrDefaultFrom args (
          lib.pipe identities [
            (lib.filter filter)
            (lib.map ({ identity, pubkey }: identity))
          ]
        );
      ...
      rageImportEncryptedOrDefault = file: default: rageImportEncryptedRaw { inherit default; } file;
    in
    {
      inherit
        rageImportEncryptedRaw
