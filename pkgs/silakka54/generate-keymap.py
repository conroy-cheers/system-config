...
def is_reportable_label(label, layer_indices):
    label = str(label)
    if label in layer_indices:
        return True
    code = qmk_keycode(label, layer_indices)
    if code.startswith("MO("):
        return True
    return bool(re.fullmatch(r"KC_F([1-9]|1[0-9]|2[0-4])", code) or code in REPORTABLE_KEYCODES)


def is_layer_key_label(label, layer_indices):
    label = str(label)
    if label in layer_indices:
        return True
    return qmk_keycode(label, layer_indices).startswith("MO(")


def key_report_masks(layers, layer_indices, key_count):
    layer_key_bits = [0] * ((key_count + 7) // 8)
    for keys in layers.values():
        for index, key in enumerate(keys):
            if is_layer_key_label(key, layer_indices):
                layer_key_bits[index // 8] |= 1 << (index % 8)

    masks = []
    for keys in layers.values():
        bits = layer_key_bits.copy()
        for index, key in enumerate(keys):
            if is_reportable_label(key, layer_indices):
                bits[index // 8] |= 1 << (index % 8)
        masks.append(bits)
    return masks


def render_key_report_masks(masks):
    return ",\n".join("    {" + ", ".join(f"0x{byte:02X}" for byte in bits) + "}" for bits in masks)


def render_key_positions(layout):
    return ",\n".join(f"    {{{entry['matrix'][0]}, {entry['matrix'][1]}}}" for entry in layout)
...
def main():
    ...
    key_report_bytes = (key_count + 7) // 8
    layers = data["layers"]
    ...
    key_report_magic_bytes = ", ".join(str(byte) for byte in KEY_REPORT_MAGIC)
    sync_magic_bytes = ", ".join(str(byte) for byte in SYNC_MAGIC)
    rendered_key_positions = render_key_positions(layout)
    rendered_key_report_masks = render_key_report_masks(
        key_report_masks(layers, layer_indices, key_count)
    )
    reported_keys_initial = ", ".join(["0xFF"] * key_report_bytes)
...
#define SILAKKA54_KEY_REPORT_BYTES {key_report_bytes}
...
static const silakka54_key_position_t silakka54_key_positions[SILAKKA54_KEY_COUNT] = {{
{rendered_key_positions}
}};
static const uint8_t silakka54_reportable_key_masks[][SILAKKA54_KEY_REPORT_BYTES] = {{
{rendered_key_report_masks}
}};
...
static uint8_t reported_key_layer = 0xFF;
static uint8_t reported_keys[SILAKKA54_KEY_REPORT_BYTES] = {{{reported_keys_initial}}};
...
static void silakka54_collect_reportable_keys(uint8_t layer, uint8_t *keys) {{
    memset(keys, 0, SILAKKA54_KEY_REPORT_BYTES);
    if (layer >= ARRAY_SIZE(silakka54_reportable_key_masks)) {{
        layer = 0;
    }}
    for (uint8_t index = 0; index < SILAKKA54_KEY_COUNT; index++) {{
        if (!(silakka54_reportable_key_masks[layer][index / 8] & (1U << (index % 8)))) {{
            continue;
        }}
        silakka54_key_position_t position = silakka54_key_positions[index];
        if (matrix_get_row(position.row) & ((matrix_row_t)1U << position.col)) {{
            keys[index / 8] |= 1U << (index % 8);
        }}
    }}
}}

static void silakka54_send_key_report(uint8_t layer, const uint8_t *keys) {{
#ifdef RAW_ENABLE
#    ifdef SPLIT_KEYBOARD
    if (!is_keyboard_master()) {{
        return;
...
#    endif
    uint8_t report[SILAKKA54_RAW_EPSIZE] = {{0}};
    const uint8_t magic[] = {{{key_report_magic_bytes}}};
    for (uint8_t i = 0; i < sizeof(magic); i++) {{
        report[i] = magic[i];
    }}
    report[7] = 1;
    report[8] = layer;
    memcpy(report + 9, keys, SILAKKA54_KEY_REPORT_BYTES);
    raw_hid_send(report, SILAKKA54_RAW_EPSIZE);
#endif
}}

static void silakka54_report_keys_if_changed(void) {{
#ifdef RAW_ENABLE
#    ifdef SPLIT_KEYBOARD
    if (!is_keyboard_master()) {{
        return;
...
#    endif
    ...
    uint8_t keys[SILAKKA54_KEY_REPORT_BYTES];
    silakka54_collect_reportable_keys(layer, keys);
    if (layer != reported_key_layer || memcmp(keys, reported_keys, sizeof(reported_keys)) != 0) {{
        reported_key_layer = layer;
        memcpy(reported_keys, keys, sizeof(reported_keys));
        silakka54_send_key_report(layer, keys);
    }}
#endif
}}
...
void keyboard_post_init_user(void) {{
    ...
    silakka54_report_keys_if_changed();
}}
        ...
        json.dumps(
            {
                ...
                "packet_version": 1,
                "key_packet_magic": KEY_REPORT_MAGIC.decode(),
                "key_report_privacy": "only layer, modifier, navigation, edit, media, function, and boot bindings are reported",
